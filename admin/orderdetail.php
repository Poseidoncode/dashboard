...
<a type="button" href="./orders.php"class="btn btn-secondary btn-sm">回到訂單資訊</a>
<a type="button" href="./order_detail_insetproduct.php?orderId=<?php echo$_GET['orderId']?>"class="btn btn-secondary btn-sm">新增商品</a>
    ...
    <table class="table table-striped border_article">
        ...
        ?>
                ...
                if($arrOrders[$i]['productImg'] != Null){
                    echo '<img class="orderImg" src="../images/products/'.$arrOrders[$i]['productImg'].'" />';
                }
                ?>
                </td>
        ...
        <tfoot>
            <!-- 列出付款方式 -->
            <?php
            $sqlpayment = "SELECT `payment_types`.`paymentTypeId`,`payment_types`.`paymentTypeName`
                             FROM `payment_types`";
            $stmtpayment = $pdo->prepare($sqlpayment);
            $stmtpayment->execute();
            // 查詢目前付款方式
            $sqlpaymentload = "SELECT `orderlist`.`paymentTypeId`,`payment_types`.`paymentTypeName`
                                FROM `orderlist`
                                INNER JOIN `payment_types`
                                ON `orderlist`.`paymentTypeId` = `payment_types`.`paymentTypeId`
                                WHERE `orderlist`.`orderId` = ?";
                
            $arrParampaymentload = [
                $_GET['orderId']
            ];
            $stmtpaymentload = $pdo->prepare($sqlpaymentload);
            $stmtpaymentload->execute($arrParampaymentload);
            if($stmtpaymentload->rowCount()>0){
                $arrload = $stmtpaymentload->fetchAll(PDO::FETCH_ASSOC)[0];
            ?>
                <tr>
                    ...
                    <td colspan="2">更改付款方式:
                        ...
                        <option value ="<?php echo $arrload['paymentTypeId']?>"><?php echo $arrload['paymentTypeName']?></option>
                        <?php 
                        if($stmtpayment->rowCount() > 0){
                            $arrpayment = $stmtpayment->fetchAll(PDO::FETCH_ASSOC);
                            for($i = 0; $i < count($arrpayment); $i++) {       
                        ?> 
                        <?php
                                if($arrload['paymentTypeId'] != $arrpayment[$i]['paymentTypeId']){
                        ?>  
                                    <option value="<?php echo $arrpayment[$i]['paymentTypeId']?>"><?php echo $arrpayment[$i]['paymentTypeName']?></option>
                        <?php
                                } 
                            }
                        }
