...
if(isset($_GET['productId'])){
    ...
    //SQL 敘述
    $sql = "SELECT `id`, `status`,`content`, `rating`, `itemId`, `created_at`, `updated_at`
            FROM `comments`
            WHERE `itemId` = ? 
    ...
    ?>
        <div class="row">
            <div class="media">
                ...
                <div class="media-body">
                    <h5 class="mt-0"><?php echo $arr[$i]["status"]; ?></h5>
        ...
        <?php
         $sqlReply = "SELECT `id`, `status`,`content`, `rating`, `itemId`, `created_at`, `updated_at`
                    FROM `comments`
                    WHERE `itemId` = ? 
        ...
        ?>
            <div class="row">
                <div class="col-md-3"><?php echo $arrReply[$j]['status'] ?>表示</div>
