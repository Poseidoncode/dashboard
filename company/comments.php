...
if(isset($_GET['id'])){
    ...
    //SQL 敘述
    $sql = "SELECT `comments`.`id`, `comments`.`status`,`comments`.`content`, `comments`.`rating`, `comments`.`itemId`, `comments`.`created_at`,`comments`.`updated_at`,`item_lists`.`productId`, `comments`.`adminReply`,`orderlist`.`memberId`,`item_lists`.`orderId`,`product`.`productName`,`product`.`companyId`,`item_lists`.`itemListId`
            FROM `comments`
        ...
        <form name="myForm" method="POST" action="./insertComments.php">
            <div class="row">
                <div class="media">
                    ...
                    <div class="media-body">
                        ...
                        <p>【會員評論】:<div><?php echo nl2br($arr[$i]["content"]); ?></div></p>
            ...
            <div class="row my-5">                    
                <div >
                    ...
                    <input class="btn-mainColor btn btn-sm my-3"  type="submit" name="smb" id="btn_insertComment_reply"  value="更新回覆內容">
                </div>
                <!-- <input type="hidden" name="commentId" value="<?php echo $arr[$i]["id"]; ?>"> -->
                
                <input class="btn-mainColor btn btn-sm my-3"  type="hidden" name="id" value="<?php echo $_GET["id"]; ?>">
